...
- **M2 RE-AUDIT round 2 (wf_462ffa57-d25, 17 agents, 1.32M tok): 12 CONFIRMED / 1 refuted — including a P1 gate bypass of the SAME class as the WU-6 P1s.** evidence/M2/audit-summary-round2.md. P1: `_find_trial_result` rglob'd the whole trial subtree and trusted any agent-writable `result.json`; Harbor copies the untrusted candidate container's `/logs/artifacts` into that subtree (proven by WU-9 manifest.json), so a malicious candidate could plant a forged reward=1.0/cost=$0 file and forge a measured pass. WU-10 fix (path-confinement to the depth-1 verifier-written trial-root result.json) dispatched via mroute execute. 6 P2 + 5 P3 queued for WU-11 hardening batch. **M2 STAYS OPEN** until WU-10 lands + re-verified (same audit→fix→reverify loop as WU-6). WU-9 oracle/no-op evidence stands (trusted agents), but the harness must be hardened before any untrusted candidate is measured.
- Fork NARROWED — free legs now DONE (2026-07-11, harness-41 cont.): [WU-8+8F] real `HarborCliClient` LANDED (see What's done). [WU-9] $0 GENUINE real-Harbor runs LANDED: oracle leg on real task `terminal-bench/regex-log` (passed=true, reward 1.0, 39s, real Docker env — engine-integration evidence, evidence/M2/measured/tb2-harbor-oracle.json) + no-op candidate leg through the full `adx measure → Tb2HarborAdapter → HarborCliClient` stack (honest quality=0.0, self_reported/raw_artifacts receipt, real job artifacts — tb2-harbor-noop.json). Both adapters now have GENUINE measured runs. ONLY the PAID leaderboard-comparable quality>0 TB2 run remains behind the operator fork. Residual wart: org/name→*name glob rewrite (WU-9F dispatched to fix client-side filename sanitize).
