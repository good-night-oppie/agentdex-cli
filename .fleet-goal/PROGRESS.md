...
- **M2 RE-AUDIT round 2 (wf_462ffa57-d25, 17 agents, 1.32M tok): 12 CONFIRMED / 1 refuted — including a P1 gate bypass of the SAME class as the WU-6 P1s.** evidence/M2/audit-summary-round2.md. ALL 12 NOW RESOLVED:
  ...
  - Each fix landed via mroute execute + coordinator review + INDEPENDENT fail-on-old/pass-on-new reproduction (same audit→fix→reverify loop as WU-6). WU-9 oracle/no-op evidence stands (trusted agents); the harness is now hardened for untrusted candidates. **M2's only remaining closure gate is the operator paid-TB2-run fork (below).**
