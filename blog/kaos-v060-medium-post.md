...
- **[MemPalace](https://github.com/milla-jovovich/mempalace)** — AAAK compact notation that compresses the proposer's context by 57% with zero quality loss, so it reads more history with fewer tokens
...
- **[EvoSkills](https://arxiv.org/abs/2604.01687)** (arXiv:2604.01687) — Surrogate Verifier: informationally isolated failure diagnostics that give the proposer structured "why it failed + how to fix it" for every harness
...
Let's walk through the full Meta-Harness workflow on a real research problem.
...
**Why this matters:** Manual review routing is a real engineering problem. A model that can reliably classify review severity lets you auto-escalate blockers, filter noise, and prioritize what gets addressed.

**Dataset:** 200 labeled PR review comments. Baseline zero-shot accuracy: **48%**.

![Full Meta-Harness demo — code review 48%→83%](https://raw.githubusercontent.com/canivel/kaos/gh-pages/docs/demos/kaos_mh_full_demo.gif)
...
That's the full benchmark contract. KAOS needs a callable that takes input and returns a score dict. The search loop handles everything else.
...
The `--background` flag spawns a detached worker process. Parent terminal dies, search continues. Poll progress with `kaos mh status` or query the SQLite database directly.
...
The search starts by evaluating three seed harnesses in parallel to establish a baseline frontier:
...
The proposer reads the archive digest — compressed to 57% of its original size by AAAK notation — and proposes a better harness each iteration.
...
Four iterations at 74%. The proposer keeps generating variations on the same theme: role-playing, few-shot combinations, explicit contrast. All converging on the same ceiling.
...
This is what CORAL calls a *stagnation-triggered redirection* — and it's the difference between a search that circles the same local maximum forever and one that actually explores the space.
...
The two-step approach breaks the plateau immediately. Instead of asking "what severity is this?", it first asks "does this identify a correctness/security problem?" and then routes accordingly. The confusion matrix clears up.
...
These skills are saved to the knowledge agent and will be available as seeds for any future search on this benchmark.
...
Here's what Meta-Harness found:
...
Every iteration, every score, every failure pattern — in your local SQLite file forever.
...
The search now tracks `stagnant_iterations` — consecutive iterations with no frontier improvement. When it crosses `stagnation_threshold`, the proposer receives a `PIVOT REQUIRED` block that lists exhausted approaches and demands structural novelty.

The cooldown is correct: pivot fires at stagnant=N, then only re-fires after another N non-improving iterations post-last-fire. No spam, no suppression.

### Tier 2 — Three-Tier Memory (attempts / notes / skills)

Inspired by CORAL's filesystem model, the search archive now has three directories:

- **`/attempts/`** — compact `{id, scores, status}` summaries of every eval. Fast proposer scanning without loading full source.
- **`/notes/`** — optional iteration observations from Claude Code: `mh_submit_candidate(..., notes="noticed the model confuses BLOCKER with IMPORTANT on short comments")`. Injected into the next digest.
- **`/skills/`** — reusable patterns written via `mh_write_skill(...)`. Persisted to the knowledge agent across searches. A skill written today becomes a seed for tomorrow's search.
...
The hub maintains `/best_per_agent/`, `/shared_skills/`, and `/shared_attempts/`. Cross-agent harnesses appear in each agent's Pareto frontier. CORAL showed this achieves 3–10× higher improvement rates vs single-agent search, with 36% of successful attempts building directly on another agent's discoveries.

---

## Knowledge Compounds Across Searches

One of the most underrated features: the knowledge agent. Every search deposits its best harnesses and extracted skills into a persistent store. The next search on the same benchmark automatically loads them as seeds.

```bash
kaos mh knowledge code_review
# [skill]  two_step_decomposition
#   Split classification into: "correctness problem?" then severity
#   Discovered at iteration 8. Broke plateau at 0.74.
#
# [skill]  attr_grounding
#   Extract impact+scope before labeling. Grounds label in attributes.
#
# [winner] two_step_attr_merged  acc=0.83
```

You're not starting over each time. You're compounding.
...
The v0.6.0 CORAL implementation covers the three core mechanisms from the paper. What's still open:

- **Real-time cross-agent note sharing** — currently hub syncs harnesses and skills; agent-to-agent notes are next
- **ARC-AGI-3 co-evolution experiments** — the benchmark is ready, multi-agent co-evolution on spatial reasoning is unexplored territory
- **Adaptive threshold tuning** — stagnation threshold that adjusts based on search history

If you run into something broken, open an issue with the `ai-reported` label — KAOS has automated triage for AI-reported issues.
