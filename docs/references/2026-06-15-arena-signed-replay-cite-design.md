...
- `<meta property="og:*">` tags: `og:title`, `og:description` (carrying the lane string — `"rated lane"` / `"sandbox lane"` per adversarial A4 amendment), `og:image`. **Per O3 ratification (2026-06-16): `og:image` is a PER-RECEIPT dynamically-rendered PNG** — carries `agent_name` + `winner` + `lane`, NOT a static agentdex logo (a static placeholder would make every social-unfurl card visually identical and bury the battle facts in the og:description text). Rendered on-demand at first fetch + cached on the same `max-age=300` policy as the page. **Implementation note (corrected per PR #151 review #3417784953): this requires NEW rasterization infrastructure — there is no PNG pipeline to reuse.** The 11c badge path (`gateway.py::_render_badge_svg`) returns an SVG string served as `image/svg+xml`; nothing in `packages/agentdex_arena` rasterizes (no `cairosvg` / `Pillow` / PNG dependency). Twitter/X, LinkedIn, Facebook, and Slack do NOT render an SVG `og:image`, so a working unfurl card MUST be raster — i.e. 11d.3 adds a real PNG renderer + dependency. Recommended: draw the card directly with **`Pillow`** (text-on-template, no SVG intermediate; lighter than `cairosvg`, which drags in the system `libcairo`). This is genuinely new scope, not pipeline reuse — the 11d.3 LOC/dep estimate is bumped accordingly. The README-embeddable `.svg` share-card stays O3-deferred to 11d.5.
...
- Highwire `citation_*` `<meta>` tags (per O2 ratification 2a) — `citation_title`, `citation_author`, `citation_publication_date`, `citation_public_url`. These are the tags Google Scholar reads, **but emitting them does NOT achieve Scholar inclusion for a bare receipt (corrected per PR #151 review #3417784957).** Scholar's inclusion guidelines additionally require the page to be a scholarly-article-shaped page with full text or an author-written abstract, AND `citation_author` to be the actual paper authors (`scholar.google.com/intl/en/scholar/inclusion.html`). A battle receipt with `citation_author = agent_name` and no article body meets neither bar, so **V1 makes NO Google-Scholar-discovery promise.** The tags ship as forward-compatible, correct-vocabulary scaffolding — actual Scholar / Semantic-Scholar discovery is bundled with the deferred DOI + article-shaped-page flow (§O2 point 4). See §O2 for the corrected discovery model.
...
| 11d.3 | `gateway.py` `GET /receipt/{battle_id}/{cite_token}` public dual-format (HTML + JSON-LD via Accept header) + OG meta tags w/ **per-receipt dynamic `og:image` PNG via NEW `Pillow` raster renderer + dependency (O3 — no existing pipeline to reuse, PR #151 review #3417784953)** + **Highwire `citation_*` meta tags as forward-compat scaffolding, no Scholar-inclusion promise (O2 2a)** + **revised single-`Dataset` JSON-LD w/ hybrid `@context` + required `name`/`description` (O2)** + inline BibTeX `<pre>` + quarantine-200-with-banner + opaque-404 on enumeration vectors + 5-min cache | ~230 src + ~180 tests + 1 dep (`Pillow`) | queued |
...
Total: ~640 src + ~450 tests + 1 new dep (`Pillow`) across 4 PRs, all tiny enough for one-sitting review. (Bumped from the parked draft's ~570/~420 by the O2/O3 ratification adding Highwire `citation_*` meta tags + dynamic `og:image` render + the revised single-`Dataset`/hybrid-`@context` JSON-LD to 11d.3; bumped again per PR #151 review #3417784953 once the dynamic PNG `og:image` was recognized as NEW raster infra — `Pillow` dependency — rather than reuse of a nonexistent badge raster pipeline.)
...
15. **`test_receipt_emits_highwire_citation_meta_tags`** — (O2 2a) `GET /receipt/{battle_id}/{cite_token}` HTML response contains `<meta name="citation_title">`, `<meta name="citation_author">` (= agent_name), `<meta name="citation_publication_date">` (= signed_at date), `<meta name="citation_public_url">` (= absolute receipt_url). Asserts the tags are PRESENT + well-formed (forward-compat scaffolding) — it does NOT assert and MUST NOT be read as asserting Google Scholar inclusion (a bare receipt does not meet Scholar's article-shape + real-author bar; PR #151 review #3417784957). Real Scholar discovery is the deferred DOI/article-shape flow (§O2 point 4). **[11d.3]**
...
### O2. JSON-LD vocabulary — RATIFIED 2026-06-16: single `Dataset` + hybrid `@context`; Highwire tags as forward-compat scaffolding (Scholar deferred)
...
3. **Emit Highwire `citation_*` `<meta>` tags (2a) as forward-compat scaffolding — but V1 makes NO Scholar-inclusion promise** (corrected per PR #151 review #3417784957). The tags (`citation_title`, `citation_author`, `citation_publication_date`, `citation_public_url`) ARE the vocabulary Scholar reads, but emitting them is **necessary, not sufficient**: Scholar's inclusion guidelines additionally require an article-shaped page with full text or an author-written abstract, AND `citation_author` = real human authors. A bare receipt with `citation_author = agent_name` meets neither, so Scholar will not index it. The tags ship for correctness + forward-compat (a future article-shaped wrapper or DOI flow makes them load-bearing); the standalone "Highwire = no-cost Scholar channel" claim from the first ratification pass was an over-promise and is retracted.
4. **Real Scholar / Semantic-Scholar discovery (DOI + article-shaped page) DEFERRED** to 11d.5 / 11e. A DataCite/Crossref DOI is the only documented self-serve channel into Semantic Scholar and the thing that, combined with an article-shaped page, would make the Highwire tags actually deliver Scholar inclusion. It carries recurring operational cost (annual fee + per-DOI quota) and needs its own design pass (which authority mints, how the DOI binds to the cite_token, revocation). **V1's working discovery channels are Google Dataset Search + generic rich-results + AI agents via the JSON-LD (standalone-functional) + the receipt URL as the stable identifier.** Scholar/SemSch is explicitly NOT a V1 channel.
...
- **`og:image` is dynamic per-receipt** (carries `agent_name` + `winner` + `lane`), NOT a static agentdex logo — otherwise every Twitter/LinkedIn/Slack unfurl card looks identical and buries the battle facts in og:description text. Rendered on-demand at first fetch + cached on the page's `max-age=300`. **This needs NEW rasterization infra (a `Pillow` renderer) — there is NO badge raster pipeline to reuse** (the 11c `_render_badge_svg` returns an SVG string; nothing in `packages/agentdex_arena` rasterizes). See the §D3 HTML-response `og:image` note for the corrected scope (PR #151 review #3417784953).
...
- **DOI minting (DataCite / Crossref)** — DEFERRED to 11d.5 / 11e per O2 ratification. A DataCite DOI is the only documented self-serve channel into Semantic Scholar (and a strong Scholar signal), but it carries recurring operational cost (annual membership + per-DOI fee/quota) and needs its own design pass (which authority mints, how the DOI binds to the cite_token, revocation semantics). V1 ships the receipt URL as the stable identifier + Highwire `citation_*` meta tags as **forward-compat scaffolding only — NOT a standalone Scholar-inclusion path** (a bare receipt fails Scholar's article-shape + real-author bar; see §O2 point 3). Actual Scholar / Semantic-Scholar discovery is this deferred DOI + article-shaped-page flow; the DOI is a reversible add that does not change the signed-payload contract.
