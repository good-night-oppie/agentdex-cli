...
def test_bridge_smoke_fixture_shape_when_present(bridge: str):
    ...
    assert not missing_top, (
        f"{bridge}_smoke.json missing top-level fields: {sorted(missing_top)}"
    )
    assert data["bridge"] == bridge, (
        f"bridge field mismatch: {data['bridge']!r} != {bridge!r}"
    )
    ...
    assert isinstance(drift, dict) and drift, (
        f"{bridge} drift_detector must be a non-empty dict — at least one "
        "fields_required_in_* key"
    )
...
def test_fixture_dir_has_readme():
    ...
    assert readme.is_file(), (
        "tests/fixtures/bridges/README.md missing — schema un-anchored"
    )
