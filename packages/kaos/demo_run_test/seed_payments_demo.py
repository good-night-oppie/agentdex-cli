"""Seed payments-demo.db with the exact engagement described in the blog."""
import os
import sqlite3

DB = "payments-demo.db"
# NOTE: we do NOT delete the DB — we reuse the one created by `kaos init`.
if not os.path.exists(DB):
    raise SystemExit(f"Run `kaos init --db {DB}` first.")
