...
class PokeAgentAdapter(LadderAdapter):
    ...
    def __init__(self, run_window: Callable[[str, float], PokeAgentResult], *, minimum_community_share: float) -> None:
    ...
    def measure(self, candidate: AgentCandidate) -> MeasureResult:
        ...
        window = self._run_window(candidate.entrypoint, candidate.budget.wall_clock_min * 60.0)
        effective = LadderClass(pokeagent_gate_class(
        ...
        return MeasureResult(
            scores={"quality": window.rating, "cost_dollar": window.cost_dollar,
                    "wall_clock_sec": window.wall_clock_sec},
            receipt=Receipt(tier="verified", kind="pokeagent_rating", ref=window.rating_ref),
            ladder_id=self.ladder_id, base_model=candidate.base_model,
            budget_usd=candidate.budget.usd, budget_wall_clock_min=candidate.budget.wall_clock_min,
            cost_is_measured=True, effective_ladder_class=effective,
        )
