...
- **Legacy-token gap (V1 known limitation, PR #130 review #3410920016, corrected per PR #137 review #3411165981):** tokens minted BEFORE the gateway started issuing the `badge_mint` scope (i.e. before the 11c.2 rollout) DO NOT carry it. `POST /badge/mint` with a legacy token returns 403 `scope 'badge_mint' not granted`. The owner has exactly one V1 workaround:
