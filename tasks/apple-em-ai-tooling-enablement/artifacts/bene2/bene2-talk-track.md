...
- **BENE 0.2.0 — shipped 2026-06-11 evening** (designed in the morning, shipped by evening — with a claims audit): 699 passing tests + 3 skipped (post-Round-3; 145 in tests/kernel/, 84 added closing A1b/A2/A4/A5), 37 MCP tools, kernel phases 4–10 done — engram substrate + compression ladder, hash-locked probes + admissibility, kill-gated promotion (PromotionBlocked), granules + fast/slow retrieval + ContextOS + pollution recovery, autonomy ladder L0–L4 enforced (L4 human-only), computed trust ledger, adapters, UI engram/trust panels, `bene demo --no-ui` in ~0.3s keyless.
...
**Situation.** Designing BENE 2.0 — designed and shipped yesterday as 0.2.0 — I argued all ten major decisions three ways before adopting any of them: as science (what experiment would prove this wrong?), as compression (what's the smallest representation that still predicts the data?), and as engineering (what breaks at the tail, and who pays for the next nine?). Not decoration: each lens is a falsifiable heuristic, and several decisions came out *differently* than a single-lens design would have (docs/design/DESIGN-RATIONALE.md).

**Action.** Hardest call — D10, rewrite vs evolve. BENE 0.1.0 was a working 445-test system at decision time. The compression lens pushed for a clean rewrite: adapters and mirrors are ugliness. The engineering lens pushed back hardest: a big-bang rewrite of a working system to chase conceptual purity is exactly the complexity that sinks projects — and the honest part is, I *wanted* the rewrite; that impulse is what the lens exists to veto. The science lens resolved it: dual-track — keep the working system green while the new kernel grows beside it, and pivot only at the proof point.

**Result.** Additive kernel plus adapters: legacy modules untouched, v2 tables additive, supersession feature-flagged and phase-numbered, with an explicit litmus — legacy suite green at every commit, `bene demo` never breaks — and an explicit horizon: adapters get *deleted* as subsystems go native. And the litmus held: 16 commits on main, legacy suite green at every one, 614 at ship, 699 after Round-3 gap-closure. The claim is architectural, not archaeological.
...
**Situation.** After leaving that role I built two agent-orchestration frameworks — KAOS v0.9.1 and BENE 0.1.0, the rebranded the 0.1.0 predecessor harness lineage. Before designing v2, the easy move was to pitch v2 on their strengths.
...
**Result.** In that prior production system the discipline gave a defensible 98.4% held-out recall@5. In 2.0 — now live — the fraction of workflows safely at L3/L4 becomes the single best maturity measure for agent tooling.
...
- Three-lens design rationale (D1–D10): `/home/admin/gh/bene-main/docs/design/DESIGN-RATIONALE.md`
...
"I built two agent-orchestration frameworks — first a production triage harness in my last role, then KAOS and BENE after I left in May. Before designing v2, I audited my own two frameworks and published 27 shortcomings with command-level evidence, mined ~100 research summaries into 48 citations, and re-architected with every decision argued three ways — science, compression, engineering. BENE 0.2.0 shipped, post-Round-3 — 699 passing tests, the kernel fully built in 16 commits with the legacy suite green at every one, closing with a claims audit that marks every remaining gap as planned, not done. And the thesis of the whole redesign is the DevEx thesis: engineers adopt agent tooling only when they trust it — so make every claim checkable. I can show you the receipts."
