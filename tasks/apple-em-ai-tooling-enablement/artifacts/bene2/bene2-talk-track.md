...
- **BENE 0.2.0 — shipped 2026-06-11 evening** (designed in the morning, shipped by evening — with a claims audit): 614 passing tests + 3 skipped (145 in tests/kernel/), 37 MCP tools, kernel phases 4–10 done — engram substrate + compression ladder, hash-locked probes + admissibility, kill-gated promotion (PromotionBlocked), granules + fast/slow retrieval + ContextOS + pollution recovery, autonomy ladder L0–L4 enforced (L4 human-only), computed trust ledger, adapters, UI engram/trust panels, `bene demo --no-ui` in ~0.3s keyless.
