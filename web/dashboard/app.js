 ...
 *   roster   fixture ./fixtures/me_agents.json      ⇄  GET /me/agents            (Bearer session token)
 *   evo      fixture ./fixtures/done.json           ⇄  GET /me/agents/<id>/evolution
 *   battle   MockLiveSource(golden-fixture frames)  ⇄  SseLiveSource(battleId,"own")  (GA-CORE-3)
 *
...
const SESSION_TOKEN = qs.get("session_token") || localStorage.getItem("agentdex_session_token") || "";
...
function liveFetchOptions() {
  return SESSION_TOKEN ? { headers: { Authorization: `Bearer ${SESSION_TOKEN}` } } : {};
}

const loadAgents = () =>
  LIVE
    ? getJSON("/me/agents", liveFetchOptions())
