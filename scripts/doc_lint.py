...
def check_doc(ctx: LintContext, path: Path) -> None:
    ...
    if fm is None:
        if is_external_verbatim and "verbatim_upstream: true" in text:
            ...
            ctx.add(path, "DOC-LINT-010", BLOCK, "missing YAML frontmatter (ep04 04-0049, 04-0051)")
            return

    # DOC-LINT-015  scope required
    if "scope" not in fm or not fm.get("scope"):
        ctx.add(
            path, "DOC-LINT-015", BLOCK, "frontmatter missing required 'scope' field (ep04 04-0304)"
        )
    else:
        scope_val = str(fm.get("scope", ""))
        if scope_val == "global" and "docs/architecture" not in rel:
            ctx.add(
                path, "DOC-LINT-015", BLOCK, "scope: global only allowed at docs/architecture/ root"
            )
    ...
    for field_name in REQUIRED_FIELDS_ALL:
        if field_name not in fm or fm.get(field_name) in (None, "", []):
            ctx.add(
                path, "DOC-LINT-010", BLOCK, f"frontmatter missing required field '{field_name}'"
            )
    ...
    if layer is None or layer == "":
        ctx.add(path, "DOC-LINT-045", BLOCK, "frontmatter missing 'layer' field (ep06 06-0130)")
    elif layer not in LAYER_VALUES:
        ctx.add(path, "DOC-LINT-045", BLOCK, f"layer='{layer}' not in {sorted(LAYER_VALUES)}")
    elif layer == "cross-cutting" and not fm.get("cross_cutting"):
        ctx.add(path, "DOC-LINT-045", BLOCK, "layer=cross-cutting requires cross_cutting: true")
    ...
    if upd is None:
        ctx.add(path, "DOC-LINT-023", WARN, "frontmatter missing 'updated' date (ep05 01-0300)")
    else:
    ...
    has_normative = any(re.search(rf"\b{re.escape(kw)}\b", body) for kw in NORMATIVE_KEYWORDS)
    ...
    for sect in ("Invariants", "Acceptance Criteria", "Definition of Done"):
        ...
        for hw in HEDGE_WORDS:
            if re.search(rf"\b{re.escape(hw)}\b", body_text, re.IGNORECASE):
                ctx.add(
                    path, "DOC-LINT-017", BLOCK, f"hedge word '{hw}' in ## {sect} (ep04 04-0344)"
                )
                break
    ...
    marker_lines = [ln for ln in nonblank if any(m in ln for m in PRIORITY_MARKERS)]
    ...
    if path.name == "AGENTS.md" and path.parent == ctx.repo_root:
        ...
        link_lines = sum(
            1 for ln in text.splitlines() if MDLINK_RE.search(ln) or WIKILINK_RE.search(ln)
        )
    ...
    if "/docs/design/" in rel or rel.startswith("docs/design/"):
        if status not in {"draft", "validated", "deprecated", "experimental"}:
            ctx.add(
                path, "DOC-LINT-027", BLOCK, "design doc missing valid status tag (ep05 01-5800)"
            )
...
def check_feature_or_spec(ctx: LintContext, path: Path, text: str, fm: dict[str, Any]) -> None:
    ...
    if not has_section(text, "Definition of Done") and not fm.get("definition_of_done"):
        ctx.add(path, "DOC-LINT-053", BLOCK, "feature/spec lacks Definition of Done (ep08 08-0313)")
...
def check_bugfix(ctx: LintContext, path: Path, text: str, fm: dict[str, Any]) -> None:
    ...
    if not has_section(text, "Reproduce"):
        ...
        if "```" not in body:
            ctx.add(
                path, "DOC-LINT-013", BLOCK, "## Reproduce missing fenced code block (ep04 04-0217)"
            )
...
def check_runbook(ctx: LintContext, path: Path, text: str, fm: dict[str, Any]) -> None:
    if not has_section(text, "Reproduce"):
        ctx.add(
            path, "DOC-LINT-013", BLOCK, "runbook lacks ## Reproduce shell block (ep04 04-0217)"
        )
    if not has_section(text, "Verify"):
    ...
    if not stateless:
        if not obs:
            ...
            if missing:
                ctx.add(
                    path, "DOC-LINT-014", WARN, f"observability missing {missing} (ep04 04-0247)"
                )
...
def check_repo_baseline(ctx: LintContext) -> None:
    ...
    if not agents_md.exists() and not agent_disabled:
        ...
        ctx.add("AGENTS.md", "DOC-LINT-025", BLOCK, "repo lacks AGENTS.md baseline (ep05 01-4500)")
    else:
        if agents_md.exists():
    ...
    has_repo_structure = (root / "docs" / "REPO_STRUCTURE.md").exists() or (
        root / "REPO_STRUCTURE.md"
    ).exists()
    has_dev_setup = (root / "docs" / "DEV_SETUP.md").exists() or (root / "DEV_SETUP.md").exists()
    if not has_ci:
        ctx.add(
            ".github/workflows/",
            "DOC-LINT-005",
    ...
    if not has_repo_structure:
        ctx.add(
            "docs/REPO_STRUCTURE.md", "DOC-LINT-005", BLOCK, "no REPO_STRUCTURE doc (ep03 01-0142)"
        )
    if not has_dev_setup:
        ctx.add("docs/DEV_SETUP.md", "DOC-LINT-005", BLOCK, "no DEV_SETUP doc (ep03 01-0142)")
    ...
    if not has_doc_lint_ci:
        ctx.add(
            ".github/workflows/", "DOC-LINT-031", BLOCK, "no CI job runs doc_lint.py (ep05 02-5500)"
        )
    ...
    if not has_sweeper and not (solo and nfiles < 50):
        ctx.add(
            ".github/workflows/doc-sweeper.yml",
            "DOC-LINT-032",
            WARN,
    ...
    for p in ("AGENTS.md", ".github/PULL_REQUEST_TEMPLATE.md", "docs/review-checklist.md"):
        if not (root / p).exists() and not solo:
            ctx.add(p, "DOC-LINT-051", WARN, f"process-encoding file '{p}' missing (ep08 08-0147)")
    if not has_ci and not solo:
        ctx.add(
            ".github/workflows/", "DOC-LINT-051", WARN, "CI workflow YAML missing (ep08 08-0147)"
        )
    ...
    if not agents_md.exists() and not agent_disabled:
        try:
            ...
            if re.search(r"Co-Authored-By:\s*(Claude|Codex|GPT|Cursor)", log, re.IGNORECASE):
                ctx.add(
                    "AGENTS.md",
                    "DOC-LINT-055",
                    BLOCK,
...
def check_commit_shape(ctx: LintContext, changed: list[Path]) -> None:
    ...
    # NOTE: get_staged_files() returns absolute paths (repo / relpath), so
    # the startswith checks must compare against the path RELATIVE to root,
    # not str(p) which is absolute.
    def _rel(p: Path) -> str:
        try:
...
def create_stub_doc(ctx: LintContext, target: Path, doc_type: str = "feature") -> None:
    ...
    header = (
        f"<!-- generated from {template.relative_to(ctx.repo_root)} by doc_lint.py --fix -->\n\n"
    )
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(header + body, encoding="utf-8")
    print(
        f"--fix: created stub {target.relative_to(ctx.repo_root)} from {template.relative_to(ctx.repo_root)}"
    )
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--staged", action="store_true", help="lint files in git index (default)")
    parser.add_argument(
        "--fix", action="store_true", help="auto-create stub docs from template when missing"
    )
    parser.add_argument("--quiet", action="store_true", help="only print BLOCK violations")
    parser.add_argument(
        "--json", dest="json_out", action="store_true", help="machine-readable JSON output"
    )
    parser.add_argument(
        "--repo", default=str(REPO_ROOT), help="repo root (default: parent of scripts/)"
    )
    parser.add_argument("targets", nargs="*", help="git-range or files/dirs to lint")
    ...
    if args.json_out:
        json.dump(
            {"violations": rendered, "exit_code": code}, sys.stdout, indent=2, ensure_ascii=False
        )
        sys.stdout.write("\n")
