    ...
    def vanilla_multi_turn_loop(
        ...
        for attempt_idx in range(num_attempts):
            if attempt_idx == 0:
                ...
                # MetaRL contains additional distill and play
                if envs.do_distillation:
                    phase_and_steps += [(attempt_idx, 'distill', 1)]
                phase_and_steps += [(attempt_idx, 'play', envs.max_turns)]

        print(phase_and_steps)
        for attempt_idx, phase, steps in phase_and_steps:
            if phase == 'distill':
                obs, infos = envs.distill()
        ...
        for steps in total_batch_list:
            ...
            for t in reversed(range(len(steps))):
                ...
                if step['phase'] == 'play':
                    ...
                    # for 'distill' phase, assign the current discounted return
                    discounted_returns[t] = running_return
