...
# After your analysis, provide a JSON skill within <skill> </skill> tags using this EXACT structure:
...
# Within <skill> </skill> tags, provide a JSON object with this EXACT structure:
...
# Here is a trajectory of the attempt. The task was {success} completed.
# {current_trajectory}

# INSTRUCTIONS:

# First, perform a deep analysis of the trajectory within <think> tags.
# - If a reference trajectory exists, compare it with the current trajectory.
# - Analyze the specific subtasks attempted (search, filter, select, purchase).
# - Determine which subtasks succeeded or failed based on the observations.
# - Identify specific actions or decisions that led to the final outcome.
# - Synthesize the 1-2 most valuable lessons regarding action selection and navigation.

# After your analysis, output the evaluation strictly in the following JSON format.
...
# --- Skill1: Reflect template with description_head field (WebShop) ---
WEBSHOP_DISTILL_TEMPLATE_WITH_DESC_HEAD = """
You are an expert evaluating a WebShop shopping attempt.
Your task is to: {task_description}
...
3. What specific actions or decisions led to this outcome?
4. What are the 1-2 most valuable lessons from this attempt?
5. In what general scenarios would these lessons be useful to a future agent?
</think>
...
"description_head": "[Describe in 1-2 sentences WHEN this lesson would be useful. Focus on the general task type, conditions, or challenges — not the specific task. e.g., 'Useful when searching for products with specific color and size constraints that require careful filter usage.' or 'Useful when the agent needs to compare prices across multiple product pages before purchasing.']"
}}
...
# --- Skill1: Query generation template (WebShop) ---
...
# --- Skill1: Re-rank template (WebShop) ---
