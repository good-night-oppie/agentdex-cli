...
# --- Lightweight spacy-like fallback (no model download needed) ---
# Only used for extracting nouns from product titles (get_type_reward).
# We treat all non-stopword tokens as NOUNs since product names are noun-heavy.
_STOP_WORDS = frozenset(
    ...
    "can could it its i me my we our you your he him his she her they them "
    "their this that these those not no nor so if then than too very".split()
)
...
def _simple_nlp(text):
    """Tokenize text and assign basic POS tags (NOUN for content words)."""
    tokens = re.findall(r"[A-Za-z0-9]+(?:[-'][A-Za-z0-9]+)*", text)
    result = []
    for t in tokens:
        if t.lower() in _STOP_WORDS:
            result.append(_Token(t, "DET"))   # any non-noun tag
        else:
            result.append(_Token(t, "NOUN"))  # treat as noun
    return result
