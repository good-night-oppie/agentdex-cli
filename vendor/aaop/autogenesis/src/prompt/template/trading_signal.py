...
AGENT_PROFILE = """
You are an innovative quantitative signal proposer and specialized in proposing single-asset strategy on cryptocurrency perpetual futures markets based on 1m data using python. You are well aware of the 0.04 perc comission fee and will take it into account.
...
2. Coding based on given instruction and format in modular manner
3. if task includes specific signal design, then the following complexity of the signals should no less than the current signal.
</intro>
"""
...
class AgentSignal(BaseSignal):
    ...
    Follow this format to generate docstring
    """
    {
        "name": "",
        "explanation": "",
    ...
    """
    Define a name here, same as the name in the docstring
    ...
    def compute(self, **kwargs) -> pd.DataFrame:
        """
        Compute ONE signal (wide format)
        ...
        data shduld be accessed as 

        self.data.close,self.data.open,self.data.high,self.data.low,self.data.volume through a pivot table,
        which are defined in __init__ by the BaseSignal and are in pandas DataFrame format with index as trade_time and columns as code.

        Rules
        -----
        - MUST NOT use future data (no look-ahead)
        - Only compute signal logic here
        - DO NOT merge / reshape
        - DO NOT create signal_1 / signal_2 naming
        - Return raw signal matrix only
        - class name is the same as the signal file name (not AgentSignal)
        """
...
SYSTEM_PROMPT = {
    "name": "trading_signal_system_prompt",
    "type": "system_prompt",
    "description": "System prompt for trading signal agents - static constitution and protocol",
    "require_grad": True,
...
AGENT_MESSAGE_PROMPT = {
    "name": "trading_signal_agent_message_prompt",
    "type": "agent_message_prompt",
    "description": "Agent message for trading signal agents (dynamic context)",
    "require_grad": False,
...
class TradingSignalSystemPrompt(Prompt):
    ...
    name: str = Field(default="trading_signal", description="The name of the prompt")
    description: str = Field(default="System prompt for trading signal agents]", description="The description of the prompt")
...
@PROMPT.register_module(force=True)
class TradingSignalAgentMessagePrompt(Prompt):
    ...
    name: str = Field(default="trading_signal", description="The name of the prompt")
    description: str = Field(default="Agent message prompt for trading signal agents", description="The description of the prompt")
