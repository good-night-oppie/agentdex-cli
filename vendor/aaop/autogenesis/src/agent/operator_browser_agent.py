...
class OperatorBrowserAgent(Agent):
    ...
    async def _get_messages(self, task: str, ctx: SessionContext = None, **kwargs) -> List[BaseMessage]:
        ...
        if self.prompt_name:
            ...
            system_prompt_name = "operator_browser_system_prompt"
            agent_message_prompt_name = "operator_browser_agent_message_prompt"
    ...
    async def __call__(self, 
        ...
        """
        Main entry point for operator browser agent through agent_manager.
        
        Args:
            task (str): The task to complete.
            files (Optional[List[str]]): The files to attach to the task.
            ctx (SessionContext): The agent context.
