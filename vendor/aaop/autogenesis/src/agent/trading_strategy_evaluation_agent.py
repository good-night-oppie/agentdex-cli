...
class TradingStrategyEvaluationAgent(Agent):
    """Trading strategy agent implementation with manual agent logic."""
    model_config = ConfigDict(arbitrary_types_allowed=True, extra="allow")
    
    name: str = Field(default="trading_strategy_evaluation", description="The name of the trading strategy evaluation agent.")
    description: str = Field(default="A trading strategy evaluation agent that can call tools to complete tasks.", description="The description of the trading strategy evaluation agent.")
    metadata: Dict[str, Any] = Field(default={}, description="The metadata of the trading strategy agent.")
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Set default prompt name for trading strategy
        if not prompt_name:
    ...
    async def _get_environment_context(self, ctx: SessionContext, record: Record = None, **kwargs) -> Dict[str, Any]:
        ...
        for env_name in ["strategy_evaluate"]:
    ...
    async def _get_messages(self, 
        ...
        self.env_name = "strategy_evaluate"
        # signal_research_images_path = workdir_path / "environment" / "signal_evaluate" / "images"

        image_folder = workdir_path / "environment" / self.env_name / "images"

        
        diagram_path = [image_folder / f for f in os.listdir(image_folder) if f.endswith((".png", ".jpg", ".jpeg", ".gif"))]
    ...
    async def __call__(self, 
        ...
        """
        logger.info(f"| 🚀 Starting TradingSignalEvaluationAgent: {task}")
