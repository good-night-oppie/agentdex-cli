...
class OfflineTradingCombinedMemory:
    ...
    async def add_event(self, event: Union[ChatEvent, List[ChatEvent]]):
        """Append events to chat history. Processing runs in background via OfflineTradingMemorySystem."""
...
class OfflineTradingMemorySystem(Memory):
    ...
    async def _process_memory_background(self, id: str) -> None:
        ...
        try:
            ...
            logger.warning(f"| ⚠️ Background offline trading memory processing failed: {e}")
        finally:
    ...
    async def end_session(self, ctx: SessionContext = None, **kwargs):
        ...
        if id in self._pending_process_tasks:
            try:
                ...
                logger.warning(f"| ⚠️ Timeout waiting for offline trading memory processing on session {id}")
            except Exception as e:
                logger.warning(f"| ⚠️ Error waiting for offline trading memory processing: {e}")
    ...
    async def get_summary(self, ctx: SessionContext = None, n: Optional[int] = None, **kwargs) -> List[OfflineTradingSummary]:
    ...
    async def get_insight(self, ctx: SessionContext = None, n: Optional[int] = None, **kwargs) -> List[OfflineTradingInsight]:
