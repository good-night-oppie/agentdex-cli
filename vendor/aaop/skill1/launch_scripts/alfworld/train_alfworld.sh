...
RUN_NAME="${RUN_NAME:-skill1_alfworld}"
export MODEL_PATH="${MODEL_PATH:-${SKILL1_ROOT}/Qwen/Qwen2.5-7B-Instruct}"
export HDFS_CHECKPOINT_PATH="${SKILL1_ROOT}/trained_models/skill1/alfworld/ckpts"
export ALFWORLD_DATA="${ALFWORLD_DATA:-${SKILL1_ROOT}/data/alfworld}"
export HYDRA_FULL_ERROR=1

MEMORY_CACHE_DIR="${SKILL1_ROOT}/trained_models/skill1/alfworld/memory_cache/${RUN_NAME}"
...
pip3 install gymnasium termcolor textworld "textworld[pddl]" 2>&1 | tail -1
...
TRAIN_CMD="python3 -X faulthandler -m verl.trainer.main_ppo \
    ...
    data.train_files=$SKILL1_ROOT/data/datasets/alfworld/train_verl.parquet \
    data.val_files=$SKILL1_ROOT/data/datasets/alfworld/test_verl.parquet \
    ...
    +env.skill_library.lambda_distill=0.3 \
    +env.skill_library.lambda_rerank=0.3 \
    ...
    trainer.project_name='skill1_alfworld' \
