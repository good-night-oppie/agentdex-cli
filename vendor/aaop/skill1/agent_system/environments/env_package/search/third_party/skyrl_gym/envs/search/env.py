...
from agent_system.environments.env_package.search.third_party.skyrl_gym.envs.search.utils import compute_score, compute_score_subem, extract_solution
...
class SearchEnv(BaseTextEnv):
    ...
    def _get_reward(self, action: str, done: bool) -> float:
        if done:
            ...
            em_score = compute_score(chat_history_str, self.ground_truth)
            # Extra metrics (stored for retrieval by envs.py)
            self._last_subem_score = compute_score_subem(chat_history_str, self.ground_truth)
            self._last_answer_found = 1.0 if extract_solution(chat_history_str) is not None else 0.0
            return em_score
        else:
            return 0
