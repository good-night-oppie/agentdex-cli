...
# pyserini/LuceneSearcher requires Java 11+ but only Java 8 is available.
# Using a BM25-based fallback search engine instead.
try:
...
class _BM25SearchEngine:
    """Fallback search engine using BM25Okapi when LuceneSearcher is unavailable (Java 11+ required)."""
    ...
    def __init__(self, products):
        ...
        corpus = [
            (p['asin'], f"{p.get('name', '')} {p.get('query', '')} {' '.join(p.get('small_description', '') if isinstance(p.get('small_description', ''), str) else ' '.join(p.get('small_description', [])))}")
            for p in products
        ]
        self._asins = [c[0] for c in corpus]
        tokenized = [c[1].lower().split() for c in corpus]
        self._bm25 = BM25Okapi(tokenized)
        self._asin_to_raw = {p['asin']: json.dumps({"id": p['asin']}) for p in products}
        print(f"[BM25 fallback] Indexed {len(products)} products")

    def search(self, query, k=50):
        scores = self._bm25.get_scores(query.lower().split())
        top_k_idx = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:k]
        return [self._Hit(self._asins[i], scores[i]) for i in top_k_idx]

    def doc(self, docid):
        return self._Doc(self._asin_to_raw.get(docid, json.dumps({"id": docid})))
...
def init_search_engine(num_products=None):
    if LuceneSearcher is not None:
        ...
        try:
            ...
            print(f"[Warning] LuceneSearcher failed ({e}), falling back to BM25")

    # BM25 fallback: build from all_products (loaded later, so we return a lazy wrapper)
    print("[Warning] Using BM25 fallback search engine (Java/Lucene unavailable)")
    return None  # will be replaced after products are loaded


def init_bm25_search_engine(products):
    """Initialize BM25 fallback after products are loaded."""
    return _BM25SearchEngine(products)
