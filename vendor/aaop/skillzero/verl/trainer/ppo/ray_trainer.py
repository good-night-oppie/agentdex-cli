...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._create_dataloader(
            train_dataset, val_dataset, collate_fn, train_sampler, test_dataset=test_dataset
        )
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler, test_dataset=None):
        """
    ...
    def _validate_once(
        self,
        use_skill=None,
        skip_generation_logging=False,
        envs=None,
        dataloader=None,
        metrics_prefix="val",
    ):
        """
        Run validation once. If use_skill is not None and val_envs supports it,
        set skill usage before running.
        Returns: (metric_dict, success_rate_dict)
        """
        val_envs = self.val_envs if envs is None else envs
        val_dataloader = self.val_dataloader if dataloader is None else dataloader
        mp = metrics_prefix

        if use_skill is not None and hasattr(val_envs, "set_use_skill"):
            val_envs.set_use_skill(use_skill)
        ...
        # Lists to collect samples for the table (only from first run when curriculum)
        sample_inputs = []
        sample_outputs = []
        sample_scores = []
        ...
        if use_skill is not None:
            desc += f" (skills={'on' if use_skill else 'off'})"
        val_progress_bar = tqdm(total=len(val_dataloader), desc=desc)
        ...
        if not skip_generation_logging:
        ...
        for data_source, rewards in data_source_reward.items():
            metric_dict[f'{mp}/{data_source}/test_score'] = np.mean(rewards)

        for data_source, tool_calls in data_source_tool_calling.items():
            metric_dict[f'{mp}/{data_source}/tool_call_count/mean'] = np.mean(tool_calls)
            # metric_dict[f'{mp}/{data_source}/tool_call_count/max'] = np.max(tool_calls)
            # metric_dict[f'{mp}/{data_source}/tool_call_count/min'] = np.min(tool_calls)

        for k, v in success_rate.items():
            metric_dict[f'{mp}/{k}'] = v
        ...
        if prompt_lengths and response_lengths:
            ...
            metric_dict[f'{mp}/prompt_length/mean'] = float(np.mean(prompt_lengths_array))
            metric_dict[f'{mp}/prompt_length/max'] = float(np.max(prompt_lengths_array))
            metric_dict[f'{mp}/prompt_length/min'] = float(np.min(prompt_lengths_array))
            
            # Response length statistics
            metric_dict[f'{mp}/response_length/mean'] = float(np.mean(response_lengths_array))
            metric_dict[f'{mp}/response_length/max'] = float(np.max(response_lengths_array))
            metric_dict[f'{mp}/response_length/min'] = float(np.min(response_lengths_array))
        ...
        if memory_visual_token_counts:
            ...
            if len(all_visual_counts) > 0:
                metric_dict[f'{mp}/memory_tokens/mean'] = float(np.mean(all_visual_counts))
                metric_dict[f'{mp}/memory_tokens/max'] = float(np.max(all_visual_counts))
                metric_dict[f'{mp}/memory_tokens/min'] = float(np.min(all_visual_counts))
        elif prompt_lengths:
            # Text mode: memory tokens = prompt_length - min(prompt_length per trajectory)
            ...
            if len(memory_text_tokens) > 0:
                metric_dict[f'{mp}/memory_tokens/mean'] = float(np.mean(memory_text_tokens))
                metric_dict[f'{mp}/memory_tokens/max'] = float(np.max(memory_text_tokens))
                metric_dict[f'{mp}/memory_tokens/min'] = float(np.min(memory_text_tokens))

        return metric_dict, success_rate

    def _validate(self):
        """Run validation. When curriculum learning is enabled, runs twice (with/without skills) and updates skill set."""
        if self.curriculum_manager is None:
            metric_dict, _ = self._validate_once()
            return metric_dict

        # Curriculum learning: validate with skills, then without, compute delta, update skills
        metric_dict, success_with = self._validate_once(use_skill=True)
        self.val_envs.set_use_skill(False)
        _, success_wo = self._validate_once(use_skill=False, skip_generation_logging=True)
        self.val_envs.set_use_skill(True)

        # Compute delta per task
        all_keys = set(success_with.keys()) | set(success_wo.keys())
        delta_success_rates = {
            k: success_with.get(k, 0.0) - success_wo.get(k, 0.0)
            for k in all_keys
        }

        # Curriculum metrics for logging
        for k in all_keys:
            if 'success_rate' in k:
                metric_dict[f'val/curriculum/with_skill/{k}'] = success_with.get(k, 0.0)
                metric_dict[f'val/curriculum/without_skill/{k}'] = success_wo.get(k, 0.0)
                metric_dict[f'val/curriculum/delta/{k}'] = delta_success_rates[k]

        max_set = self.curriculum_manager.get_current_max_set(self.global_steps)
        new_skills = self.curriculum_manager.update_skill_set(
            delta_success_rates, max_set=max_set
        )
        metric_dict['val/curriculum/active_skill_count'] = len(
            self.curriculum_manager.get_active_skill_names()
        )
        metric_dict['val/curriculum/active_skills'] = ','.join(
            self.curriculum_manager.get_active_skill_names()
        )
        metric_dict['val/curriculum/max_set'] = max_set

        # Update skills on train / val / test envs
        if hasattr(self.envs, 'update_skills'):
            self.envs.update_skills(new_skills)
        if hasattr(self.val_envs, 'update_skills'):
            self.val_envs.update_skills(new_skills)
        if self.test_envs is not None and hasattr(self.test_envs, 'update_skills'):
            self.test_envs.update_skills(new_skills)
    ...
    def fit(self):
        ...
        # Optional: evaluate only on test split (data.test_files / test_envs), then exit
        if self.config.trainer.get("test_only", False):
            assert self.val_reward_fn is not None, "test_only requires val_reward_fn"
            assert self.test_dataloader is not None, "test_only requires data.test_files"
            assert self.test_envs is not None, "test_only requires test_envs (e.g. search with test_files)"
            if self.curriculum_manager is not None and hasattr(self.test_envs, "update_skills"):
                self.test_envs.update_skills(self.curriculum_manager.get_full_skills())
            use_skill_flag = self.config.env.get("use_skill", False)
            test_metrics, _ = self._validate_once(
                envs=self.test_envs,
                dataloader=self.test_dataloader,
                metrics_prefix="test",
                use_skill=use_skill_flag,
            )
            pprint(f"test_only metrics: {test_metrics}")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if is_last_step and self.config.trainer.get("test_after_train", False):
                    if (
                        self.val_reward_fn is not None
                        and getattr(self, "test_dataloader", None) is not None
                        and self.test_envs is not None
                    ):
                        # Final test: all skill files merged (curriculum-unpruned), then no-skill run
                        if self.curriculum_manager is not None and hasattr(
                            self.test_envs, "update_skills"
                        ):
                            self.test_envs.update_skills(
                                self.curriculum_manager.get_full_skills()
                            )
                        test_metrics_full, _ = self._validate_once(
                            envs=self.test_envs,
                            dataloader=self.test_dataloader,
                            metrics_prefix="test/full_skill",
                            use_skill=True,
                        )
                        test_metrics_wo, _ = self._validate_once(
                            envs=self.test_envs,
                            dataloader=self.test_dataloader,
                            metrics_prefix="test/no_skill",
                            use_skill=False,
                            skip_generation_logging=True,
                        )
                        logger.log(
                            data={**test_metrics_full, **test_metrics_wo},
