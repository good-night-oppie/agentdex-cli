...
Source of truth: `docs/design/CLAIMS-AUDIT.md`. **Implemented:** engram ladder + provenance + lineage + FTS; probes/locks/verdicts; experiments journal; trust ledger + weighted tally; genomes/mutation/Pareto/distillation/genes; kill-gated promote; granules/consolidate; adaptive retrieval; ContextOS; pollution detect+recover; autonomy ladder; senses; sweeper; loop guards (standalone); adapters; demo/UI panels.
**PLANNED (do NOT claim):** plasticity/outcome-weighted retrieval ranking; critical-step failure localizer; continuous-quality outcome signal; runner wiring of ContextOS and loop guards into the live agent loop; scheduled/nightly consolidation; `kernel:` section in bene.yaml; VEA evidence re-highlighting; deterministic replay tooling; in-episode harness mutation; MemGAS entropy routing.
