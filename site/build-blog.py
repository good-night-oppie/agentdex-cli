...
def build() -> None:
    # Collect sources BEFORE touching output. A zero-source run is almost always
    # a missing-sources misconfiguration (the top-level blog/ tree isn't in this
    # checkout) rather than an intentional "delete every post": the old code
    # rmtree'd OUT/ZH_OUT first and unconditionally, so running build-blog.py
    # without sources replaced the committed blog pages (what-is-bene.html, …)
    # with empty indexes — the site could no longer be reproduced from sources.
    # Refuse to wipe committed output when there is nothing to rebuild it from
    # (PR #267 review 3433380724). Deleting the LAST post is a deliberate act:
    # remove its source and the rebuild drops it; an empty tree is the error.
    en_posts = collect(BLOG)
    zh_posts = collect(BLOG / "zh")
    if not en_posts and not zh_posts:
        raise SystemExit(
            f"build-blog: no blog sources found under {BLOG} — refusing to wipe "
            f"committed output at {OUT} / {ZH_OUT}. Commit the blog/ sources or run "
            f"from a tree that has them."
        )

    # Clear stale output so a renamed/deleted post's old <slug>.html can't linger
    # and keep being deployed (the index would no longer list it). Reached only
    # once sources exist, so the build stays reproducible from sources alone.
